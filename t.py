...
def dataInterpretation(rawData, addr):
    ...
    if (dataSplited[0] == b'firstConnection'):
        ...
        connectionSocket = receivingSocket.accept()
...
"""
send a Image to a node of the network
"""
...
def sendImage():
    ...
    try:
        imageName = input('Enter the Name of the image with its ending: ')
        file = open(imageName, 'rb')
        image_data = file.read(2048)

        clientSocket.connect((selfIPAddress, networkPort)) # connects to the server
